...
testAsync("useEpicReducer should reduce", finished => {
  ...
  let btn = tree |> RT.getByText(~matcher=`Str("DO SOMETHING"),_);
  let container = tree |> RT.container
  let options = DT.WaitForElement.makeOptions(~container, ());
  ignore(
    DT.waitForElement(
      ~options,
      ~callback=() => tree |> RT.getByText(~matcher=`Str("DID SOMETHING"),_),
      (),
    )
    |> Js.Promise.then_(_ =>
         Js.Promise.resolve(
...
testAsync("useEpicReducer should be epic", finished => {
  ...
  ignore(
    DT.waitForElement(
      ~options,
      ~callback=() => tree |> RT.getByText(~matcher=`Str("DID OTHER THING"),_),
      (),
    )
    |> Js.Promise.then_(_ =>
         Js.Promise.resolve(
